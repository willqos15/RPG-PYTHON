...
def criarjc():
        ...
        def bconfirmar():
                ...
                if mraca.get() != 'Escolha uma raça':
                    ...
                    if mraca.get() == 'Humano' and contador==0:
                        ...
                        lbhum = Label(atrihum, text='\n\nOBS: Você tem +2 pontos raciais para\ndistribuir em dois atributos diferentes.')
                        lbhum.pack(side=TOP)

                        lbdois1 = Label(atrihum, text='  +2', fg='green')
                        lbdois1.pack(side=LEFT)
                        vatrihum1 = StringVar(atrihum)
                        vatrihum1.set('Escolha')
                        w = OptionMenu(atrihum, vatrihum1, 'Força', 'Inteligência', 'Carisma', 'Constituição', 'Sabedoria', 'Destreza')
                        w.pack(side=LEFT)

                        lbdois2 = Label(atrihum, text='  +2', fg='green')
                        lbdois2.pack(side=RIGHT)
                        vatrihum2 = StringVar(atrihum)
                        vatrihum2.set("Escolha")
                        w = OptionMenu(atrihum, vatrihum2, 'Força', 'Inteligência', 'Carisma', 'Constituição','Sabedoria', 'Destreza')
                        w.pack(side=RIGHT)

                        okhum = Button(atrihum, text='Confirmar',command= confirmahuman)
                        okhum.place(x=110,y=200)
                ...
                if vatri1.get() and vatri2.get() and vatri3.get() and vatri4.get() and vatri5.get() and vatri6.get() != 'Escolha' and sorted(lista) != sorted(listaAtri):
        ...
        jcadastro.geometry("750x260+350+200")
        ...
        lbnome.grid(row=0, column=5, sticky=W, padx=(10,0))
        enome = Entry(jcadastro, width=30)
        enome.grid(row=1, column=5, sticky=E, padx=(10,10))
