...
def criarjlp():
    ...
    jlistaperson.geometry("850x412+350+100")
    ...
    def estatistica():
        ...
        jestatistica.geometry("900x500+350+100")
        ...
        psm = ('%d (%s%%) %s' % (
            qtdsm, ("{0:.2f}".format(round((100 * qtdsm / sum(xlista)), 2))), label_listasexo[0]))
        psf = ('%d (%s%%) %s' % (
            qtdsf, ("{0:.2f}".format(round((100 * qtdsf / sum(xlista)), 2))), label_listasexo[1]))


        fig1 = plt.figure(figsize=(8.5, 4))
        plt.subplot(1, 2, 1)
        plt.pie(xlista, labels=label_listasexo, autopct=lambda p: '{:,.0f}'.format(p * sum(xlista) / 100), colors=cores)
        plt.title('Sexo\n')
        plt.axis('equal')
        ...
        # Formatação legenda porcentagens raça
        ph = ('%d (%s%%) %s' % (
            qtdh, ("{0:.2f}".format(round((100 * qtdh / sum(xlista)), 2))), label_listaraca[0]))
        pe = (
            '%d (%s%%) %s' % (
                qtde, ("{0:.2f}".format(round((100 * qtde / sum(xlista)), 2))), label_listaraca[1]))
        pa = ('%d (%s%%) %s' % (
            qtda, ("{0:.2f}".format(round((100 * qtda / sum(xlista)), 2))), label_listaraca[2]))
        po = ('%d (%s%%) %s' % (
            qtdo, ("{0:.2f}".format(round((100 * qtdo / sum(xlista)), 2))), label_listaraca[3]))

        fig1 = plt.figure(figsize=(8.5, 4))
        plt.subplot(1, 2, 1)
        plt.pie(xlista, labels=label_listaraca, autopct=lambda p: '{:,.0f}'.format(p * sum(xlista) / 100), colors=cores)
        plt.title('Raça\n')
        plt.axis('equal')
        plt.rcParams['legend.fontsize'] = 9
        ...
        cores = ('red','blue', 'yellow','lime')

        # Formatação legenda porcentagens classe
        pg = ('%d (%s%%) %s' % (
            qtdg, ("{0:.2f}".format(round((100 * qtdg / sum(xlista)), 2))), label_listaclasse[0]))
        pm = (
            '%d (%s%%) %s' % (
            qtde, ("{0:.2f}".format(round((100 * qtdm / sum(xlista)), 2))), label_listaclasse[1]))
        pl = ('%d (%s%%) %s' % (
            qtdl, ("{0:.2f}".format(round((100 * qtdl / sum(xlista)), 2))), label_listaclasse[2]))
        pd = ('%d (%s%%) %s' % (
            qtdd, ("{0:.2f}".format(round((100 * qtdd / sum(xlista)), 2))), label_listaclasse[3]))

        fig1 = plt.figure(figsize=(8.5, 4))
        plt.subplot(1, 2, 1)
        plt.pie(xlista, labels=label_listaclasse, autopct=lambda p: '{:,.0f}'.format(p * sum(xlista) / 100), colors=cores)
        plt.title('Classe\n')
        plt.axis('equal')
        plt.rcParams['legend.fontsize'] = 9
        ...
        phg = ('%d (%s%%) %s' % (
            qtdhguerreiro, ("{0:.2f}".format(round((100 * qtdhguerreiro / sum(xlista)), 2))), label_listarc[0]))
        phm = (
            '%d (%s%%) %s' % (qtdhmago, ("{0:.2f}".format(round((100 * qtdhmago / sum(xlista)), 2))), label_listarc[1]))
        phl = ('%d (%s%%) %s' % (
            qtdhladino, ("{0:.2f}".format(round((100 * qtdhladino / sum(xlista)), 2))), label_listarc[2]))
        phd = ('%d (%s%%) %s' % (
            qtdhdruida, ("{0:.2f}".format(round((100 * qtdhdruida / sum(xlista)), 2))), label_listarc[3]))
        peg = ('%d (%s%%) %s' % (
            qtdeguerreiro, ("{0:.2f}".format(round((100 * qtdeguerreiro / sum(xlista)), 2))), label_listarc[4]))
        pem = (
            '%d (%s%%) %s' % (qtdemago, ("{0:.2f}".format(round((100 * qtdemago / sum(xlista)), 2))), label_listarc[5]))
        pel = ('%d (%s%%) %s' % (
            qtdeladino, ("{0:.2f}".format(round((100 * qtdeladino / sum(xlista)), 2))), label_listarc[6]))
        ped = ('%d (%s%%) %s' % (
            qtdedruida, ("{0:.2f}".format(round((100 * qtdedruida / sum(xlista)), 2))), label_listarc[7]))
        pag = ('%d (%s%%) %s' % (
            qtdaguerreiro, ("{0:.2f}".format(round((100 * qtdaguerreiro / sum(xlista)), 2))), label_listarc[8]))
        pam = (
            '%d (%s%%) %s' % (qtdamago, ("{0:.2f}".format(round((100 * qtdamago / sum(xlista)), 2))), label_listarc[9]))
        pal = ('%d (%s%%) %s' % (
            qtdaladino, ("{0:.2f}".format(round((100 * qtdaladino / sum(xlista)), 2))), label_listarc[10]))
        pad = ('%d (%s%%) %s' % (
            qtdadruida, ("{0:.2f}".format(round((100 * qtdadruida / sum(xlista)), 2))), label_listarc[11]))
        pog = ('%d (%s%%) %s' % (
            qtdoguerreiro, ("{0:.2f}".format(round((100 * qtdoguerreiro / sum(xlista)), 2))), label_listarc[12]))
        pom = (
            '%d (%s%%) %s' % (qtdomago, ("{0:.2f}".format(round((100 * qtdomago / sum(xlista)), 2))), label_listarc[13]))
        pol = ('%d (%s%%) %s' % (
            qtdoladino, ("{0:.2f}".format(round((100 * qtdoladino / sum(xlista)), 2))), label_listarc[14]))
        pod = ('%d (%s%%) %s' % (
            qtdodruida, ("{0:.2f}".format(round((100 * qtdodruida / sum(xlista)), 2))), label_listarc[15]))

        fig1 = plt.figure(figsize=(8.5, 4))
        plt.subplot(1, 2, 1)
        plt.pie(xlista, labels=label_listarc, autopct=lambda p: '{:,.0f}'.format(p * sum(xlista) / 100), colors=cores)
